pro ftsread,data,wl,nblocks,xlam=xlam,dir=tdir,plot=iplot,sdir=sdir
;+
; NAME:
;	FTSREAD
; PURPOSE:
...
;				English messages.
;       data files on $IDL_DIR/data/kis_lib
;                 19/8/93 Reinhold Kroll, IAC
;       data files on $IDL_LOCAL/data/kis_lib
;		28/1/99
;---
...
if(n_elements(sdir) eq 0) then sdir=getenv('SPECTRAL_DATA')
;if host eq 'goofy' then s_host='goofy' else s_host='daisy'
; set the full path name of FTS-atla file::
;if sdir eq '' then $
;   if host eq 'goofy' then sdir='/dat/goofy_2/spectral_atlas' else $
;                           sdir='/dat/daisy_2/spectral_atlas'
;sdir=getenv('IDL_LOCAL')+'/data/'
...
nblocks = ceil(nblocks)

skip=fix(wl-3290)
cmd='dd if='+sdir+'/fts_cent.dat of='+tdir+ $
    '/tmp.fts bs=1000 skip='+strtrim(skip,1)+' count='+strtrim(nblocks,1)
;if s_host ne 'goofy' then cmd='rsh '+s_host+' '+cmd
print,cmd
spawn,cmd

xlam=findgen(500l*nblocks)*.002+wl
data=intarr(500l*nblocks)
openr,unit,tdir+'/tmp.fts',/get_lun
readu,unit,data
if(!version.arch eq "alpha" or !version.arch eq "x86_64") then byteorder,data
free_lun,unit
spawn,'rm '+tdir+'/tmp.fts'
